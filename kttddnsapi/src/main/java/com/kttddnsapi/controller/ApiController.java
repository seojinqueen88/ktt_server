...
public class ApiController {
  ...
  public String KTTDDNS_SYSID_TO_DEVICEINFO(Map<String, Object> request,
    ...
    List<Map<String, Object>> deviceListOrg =
        apiService.selectDeviceInSysidForDeviceinfo(sysidStrings);
    List<Map<String, Object>> deviceList = new ArrayList<>();
  ...
  public String KTTDDNS_SERVICENO_TO_OSS_DEVICEINFO(Map<String, Object> request,
      Map<String, Object> response) {
    ...
    List<Map<String, Object>> deviceListOrg =
        apiService.selectDeviceWhereServicenoForOss(serviceNo);
    List<Map<String, Object>> deviceList = new ArrayList<>();
    for (Map<String, Object> deviceItem : deviceListOrg) {
      if ((int) deviceItem.get("domainType") == 20 || (int) deviceItem.get("domainType") == 21)
        continue;
  ...
  public String KTTDDNS_SYSID_TO_DEVICEINFO2(Map<String, Object> request,
      Map<String, Object> response) {
    @SuppressWarnings("unchecked")
    ArrayList<String> sysidList = (ArrayList<String>) request.get("sysidList");
    ...
    List<Map<String, Object>> deviceListOrg =
        apiService.selectDeviceInSysidForDeviceinfo2(sysidStrings);
    List<Map<String, Object>> deviceList = new ArrayList<>();
    for (Map<String, Object> deviceItem : deviceListOrg) {
      if ((int) deviceItem.get("domainType") == 20 || (int) deviceItem.get("domainType") == 21)
        continue;
    ...
    for (Map<String, Object> deviceItem : deviceList) {
      ...
      deviceItem.put("masterKey",
          Encryptions.encryptMasterkey(Encryptions.getSHA(noColonMac + "KTT_MASTER", 1)));
    }
    response.put("deviceList", deviceList);
    return "success";
  }
   ...
   */
  /**
   * SW 주장치 연동 및 영상 시스템 고도화 개발 시방서 V3 (002) 에 따라
   * OTP_YN 이 1 인 경우와 register_type이 0~2 이면서 servie_open - 2이상인 장비에 대하여 장비 리스트를 응답
   * 해당 장비 펌웨어 버전이 OTP 일괄인증을 지원하는 경우에는 해당 정보를 같이 반영한다.
   */
  ...
  public String KTTDDNS_SERVICENO_TO_APP_DEVICEINFO(Map<String, Object> request, Map<String, Object> response)
  {
	  ...
	  final Boolean ignoreOTPCondition = request.containsKey("ignoreOTP") ? Boolean.valueOf( request.get("ignoreOTP").toString()) : Boolean.FALSE ;
    ...
    /*
     * String serviceNoListString = ""; for (int i = 0; i < serviceNoList.size(); i++) { String
     * serviceNo = serviceNoList.get(i); if (serviceNo.length() == 0) continue; if
     * (serviceNoListString.length() == 0) serviceNoListString += ("'" + serviceNo + "'"); else
     * serviceNoListString += (", '" + serviceNo + "'"); } if (serviceNoListString.length() == 0) {
     * List<Map<String, Object>> deviceList = new ArrayList<>(); response.put("deviceList",
     * deviceList); deviceList = null; return "success"; }
     * 
     */
    ...
    try 
    {
    	...
    	// 마지막 ,(쉼표)제거
    	serviceNoListString.delete(serviceNoListString.toString().length() - 1, serviceNoListString.toString().length());
    ...
    List<Map<String, Object>> deviceListOrg = apiService.selectDeviceWhereInServicenoForApp(serviceNoListString.toString()); 
    // 위 기능을 위하여 해당 함수도  일부 수정
    
    /*
     * 이전 코드 oty_yn을 확인하지 않는 코드 final List<Map<String, Object>> deviceList = new ArrayList<>(); 
     * for(Map<String, Object> deviceItem : deviceListOrg) 
     * { int domainType = (int) deviceItem.get("domainType"); int otp_yn = (int) deviceItem.get("otp_yn"); 
     * if (domainType == 20 || (int) domainType == 21) continue; // 2022-11-30일 전에는 아래 조건 판단이 없이 리스트를 전달하였음. 
     * if (otp_yn <= 0 || otp_yn > 3) // otp_yn : 사용안함(0), 기존사용자(1) , 관리자 변경 (2) , 인증 완료(3) //
     * continue;
     * 
     * deviceList.add(deviceItem); } for (Map<String, Object> deviceItem : deviceList) { String
     * noColonMac = (String) deviceItem.get("macAddress"); deviceItem.put("masterKey",
     * Encryptions.encryptMasterkey(Encryptions.getSHA(noColonMac + "KTT_MASTER", 1))); }
     */
    ...
    if (deviceListOrg != null) 
    {
        ...
        if (deviceItem != null && deviceItem.containsKey("domainType")) 
        {
        	...
        	if ((domainType == 22 || domainType == 23)) // 'octdvr.co.kr'  20    'octnvr.co.kr'  21
        	{
        		...
        		b = b || (service_open == 2 && (register_type == 0 || register_type == 1 || register_type==2)) ;//resister_type check
                ...
                // String noColonMac = (String) deviceItem.get("macAddress");
                  ...
                  /* String strModelName = deviceItem.containsKey("modelName") ?  deviceItem.get("modelName").toString(): "";
                  if(tDeviceVer != 0 && strModelName.length() > 0){IniEditor manual_notice_list = new IniEditor();
                    RequestAttributes requestAttributes = RequestContextHolder.getRequestAttributes();
                    HttpServletRequest httpServletRequest = ((ServletRequestAttributes) requestAttributes).getRequest();
                    String OTP_B_A_INI = httpServletRequest.getServletContext().getRealPath("opt_b_a.ini");
                    System.out.println("otp_batch_authentication_avaiable" + OTP_B_A_INI);
                    File file = new File(OTP_B_A_INI);
                    if(file.exists()) {try {manual_notice_list.load(file);
                        String fwAvailbleVer = manual_notice_list.get(strModelName, "fwver");
                        if(fwAvailbleVer.length()>0 && tDeviceVer >= apiService.verString2Int(fwAvailbleVer.trim()))
                        {deviceItem.put("otp_batch_authentication_avaiable", true);}
                      } catch (IOException e) {deviceItem.put("otp_batch_authentication_avaiable", false);}
                    }else{deviceItem.put("otp_batch_authentication_avaiable", true);}}
                  else {deviceItem.put("otp_batch_authentication_avaiable", false);} */
        				}
        			}
        			deviceList.add(deviceItem);
        			noColonMac = null;
  ...
  public String KTTDDNS_SYSID_TO_DEVICEINFO3(Map<String, Object> request,
      Map<String, Object> response) {
    @SuppressWarnings("unchecked")
    ArrayList<String> sysidList = (ArrayList<String>) request.get("sysidList");
    ...
    List<Map<String, Object>> deviceList =
        apiService.selectDeviceInSysidForDeviceinfo3(sysidStrings);
    response.put("deviceList", deviceList);
    return "success";
  }

  public String KTTDDNS_SERVICENO_TO_CS_DEVICEINFO(Map<String, Object> request,
      Map<String, Object> response) {
    @SuppressWarnings("unchecked")
    ...
    List<Map<String, Object>> deviceList =
        apiService.selectDeviceWhereInServicenoForCs(serviceNoListString);
    response.put("deviceList", deviceList);
    return "success";
  }
  ...
  /*
   * public String KTTDDNS_DOMAIN_TO_DEVICEINFO(Map<String, Object> request, Map<String, Object>
   * response) {
   * 
   ...
   * 
   * if(device == null) { Map<String, Object> nDevice = new HashMap<>(); nDevice.put("fullDomain",
   * (domainList.get(i)).toLowerCase()); nDevice.put("p2pUid", ""); nDevice.put("p2pPriority", 0);
   * nDevice.put("macAddress", ""); deviceList.add(i, nDevice); } else { String noColonMac =
   * device.get("macAddress").toString(); P2pregister p2pregister = apiService.getP2p(noColonMac);
   * device.put("p2pUid", p2pregister.getP2p_uid()); device.put("p2pPriority",
   * p2pregister.getP2p_priority()); deviceList.add(i, device); } }
   * 
   * response.put("deviceList", deviceList); return "success"; }
   */

  public String KTTDDNS_MAC_TO_DEVICEINFO(Map<String, Object> request,
      Map<String, Object> response) {
 ...
 //selectDevicePublicIpWhereMac -> service user 가 1인 경우에 한정하여 발급
  ...
  public String KTTDDNS_REG_P2P(Map<String, Object> request, Map<String, Object> response) {
    ...
    if (device.get("p2pUid").toString().length() == 0) {
      ...
      if (!apiService.updateDeviceP2pWhereMac(ddnsMac, device.get("p2pUid").toString(),
          p2pPriority))
        return "fail";
    }
  ...
  public String KTTDDNS_SYSID_TO_DEL_DOMAIN(Map<String, Object> request,
      Map<String, Object> response) {
  ...
  public String KTTDDNS_DOMAIN_TO_DEL_DOMAIN(Map<String, Object> request,
      Map<String, Object> response) {
  ...
  public String KTTDDNS_MAC_TO_DEL_DOMAIN(Map<String, Object> request,
      Map<String, Object> response) {
    String macAddress = (String) request.get("macAddress");
   ...
   * @작성일 : 2022. 11. 30.
   * @작성자 : Foryoucom
   * @변경이력 : 2022.11.30 dB서버 내 MAC 주소가 있는지 검사 및 있는 경우 해당 장비의 버전을 확인하여 그에 따라서 OTP 연산을 수행
   * @Method 설명 :
   * @param request
   * @param response
   * @return
   */
  public String KTTDDNS_MAC_TO_INSTANTPW(Map<String, Object> request,
      Map<String, Object> response) {
    ...
    String DeprecatedCheckFile =
        httpServletRequest.getServletContext().getRealPath("DeprecatedCheckFile");
    ...
    /*
     * if (macAddress == null || staffId == null) { // logger.debug("MAC : NULL, stattfId : NULL");
     * return "fail"; }
     */
    String apiMac = Encryptions.remakeMac(macAddress, false);
    if (apiMac.length() != 12) {
    ...
    if (request.containsKey("version") == false) {
      if (isDeprecatedOldFunction == false) {
        instantPw = apiService.ktt_cipher_makeInstantPasswd(year, month, day, hour, mac_num, 1); // use new style
                                                                                                 // password
      } else {
      ...
      if (device != null) // MAC을 기준으로 조회가 된 경우
      {
        ...
        try {
          ...
          if (device.containsKey("last_ver") == false
              || device.containsKey("device_ver") == false) {
            msg = "모델을 확인할 수 없습니다.";
          ...
          String strLast_ver =
              device.get("last_ver") != null ? device.get("last_ver").toString() : "";
          String strDevice_ver =
              device.get("device_ver") != null ? device.get("device_ver").toString() : "";
          ...
          {
            instantPw = apiService.ktt_cipher_makeInstantPasswd(year, month, day, hour, mac_num, 0); // use  old
            // style
            // password
          } else {
      ...
      } else // MAC을 기준으로 조회가 되지 않은 경우
      {
        try {
          ...
          switch (version) {
            case 1: // 신형 OTP
              instantPw =
                  apiService.ktt_cipher_makeInstantPasswd(year, month, day, hour, mac_num, 1); // use new
                                                                                               // style password
              break;
            case 2: // 구형 OTP
              instantPw =
                  apiService.ktt_cipher_makeInstantPasswd(year, month, day, hour, mac_num, 0); // use old
                                                                                               // style password
              break;
            default:
   ...
   * @param request
   * @param response
   * @return //userLevel //0 : 영상앱에서 사용자가 호출할 때 //1 : OSS앱에서 KTT직원이 호출할 때 //userLevel 키가 없을때는 0으로
   *         판단한다. (영상앱에서는 userLevel을 추가하지 않을 예정) //
   */
  public String KTTDDNS_REMOTEAS_ASCODE(Map<String, Object> request, Map<String, Object> response) {
    ...
    response.put("macAddress", macAddress);
    response.put("msg",
        "[우클릭] -> [기타] -> [DMS]에서 AS Code를 입력해주세요. 입력창이 없을 경우 펌웨어를 V2.0.0 이상으로 업그레이드 해주세요.");
    return "success";
  }
   ...
   * @return
   * 
   *         //isPrivate 이 0 이고 self_as(user_level) 이 1 이면 실패를 리턴한다.(fail) isPrivate --> 0 : AS
   *         Code를 Public IP에서 사용(기본값) ,1 : AS Code를 Local GUI 또는 Private IP에서 사용 //isPrivate 이 1 이고
   *         self_as(user_level) 이 2이면 실패를 리턴한다.(fail) //isPrivate 이 0 이고 self_as(user_level) 이 2일 때
   *         발급 시간이 10분(OTP유효시간)을 초과한 경우 발급 정보를 DB에서 삭제하고 실패를 리턴한다.(timeout)
   */
  ...
  public String KTTDDNS_CHECK_ASCODE(Map<String, Object> request, Map<String, Object> response) {
    ...
    if (macAddress == null || asCode == null
        || (isPrivate != null && isPrivate != 0 && isPrivate != 1)
    ...
    for (Selfas selfasItem : selfasList) {
      ...
      if (LocalDateTime.now().isAfter(
          ((selfasItem.getCertnum_create_time()).toLocalDateTime()).plusMinutes(60 * 24))) {
        ascode_del_check =
            selfasService.deleteSelfasWhereMacUserlevel(apiMac, selfasItem.getUser_level());
        return "timeout";
      }
     ...
     final int checkAscodeTime = (int) setup.get("devplaykey_limit_min");
      ...
      if (isPrivate == 0 && selfasItem.getUser_level() == 2 && LocalDateTime.now().isAfter(
          ((selfasItem.getCertnum_create_time()).toLocalDateTime()).plusMinutes(checkAscodeTime))) {
  ...
  public String KTTDDNS_DEVICE_CMD_URL(Map<String, Object> request, Map<String, Object> response) {
    ...
    String url = "https://" + device.get("fullDomain") + ":" + device.get("webport")
        + "/kttapi/remotecmd?devkey=" + devKey;
...
//    File file = new File(context.getRealPath("/WEB-INF/classes/deprecated_KTTDDNS_CHECK_DEVKEY"));
 //   boolean isDeprecatedOldFunction = file.exists();
 //   file = null;
...
//    }
  ...
  public String KTTDDNS_DEVICE_PLAY_URL(Map<String, Object> request, Map<String, Object> response) {
    ...
    try {
      ...
      if (now.compareTo(limitTime) == 1) {
        response.put("msg",
            "영상관제 가능 기간(설정시간:" + (int) setup.get("eventtime_limit_day") + "일)이 초과되었습니다.");
    ...
    for (Map<String, Object> deviceItem : deviceListOrg) {
      ...
      String url = "https://" + deviceItem.get("fullDomain") + ":" + deviceItem.get("webport")
          + "/kttapi/eventplaycmd?devplaykey=" + devPlayKey + "&eventtime=" + eventTime
  ...
  public String KTTDDNS_SERVICENO_TO_DEVICE_PLAY_URL(Map<String, Object> request,
      Map<String, Object> response) {
    String serviceNo = (String) request.get("serviceNo");
    String eventTime = (String) request.get("eventTime");
    Integer guardStatus = (Integer) request.get("guardStatus");
    ...
    if (serviceNo == null || serviceNo.length() != 8 || eventTime == null
        || eventTime.length() != 14 || !StringUtils.isNumeric(eventTime) || empNo == null
        || empNo.length() == 0) {
      response.put("msg", "포맷 에러 등으로 실패하였습니다.");
      return "fail";
    }
  ...
  public String KTTDDNS_CHECK_DEVPLAYKEY(Map<String, Object> request,
      Map<String, Object> response) {
    String macAddress = (String) request.get("macAddress");
  ...
  public String KTTDDNS_DEVICE_RESET_PW_URL(Map<String, Object> request,
      Map<String, Object> response) {
    String sysid = (String) request.get("sysid");
    String empNo = (String) request.get("empNo");
  ...
  public String KTTDDNS_OTP_LOG(Map<String, Object> request, Map<String, Object> response) {
    ...
    String msg = "OTP SEND - DOMAIN : " + domain + ", MAC ADDRESS : " + macAddress
        + ", SYSTEM ID : " + sysid + ", PHONE : " + phone;
     ...
     * String rtn_msg = "fail";
     * 
     * if(domain == null || sysid == null || macAddress == null || phone == null) return rtn_msg;
     * 
     * String fileNameDate = new SimpleDateFormat("yyyy-MM-dd").format(System.currentTimeMillis());
     * String logDate = new SimpleDateFormat("HH:mm:ss").format(System.currentTimeMillis()); String
     * logMsg = logDate + " | " + domain + " | " + macAddress + " | " + sysid + " | " + phone;
     * String savePath = "/usr/local/otp_log"; File folder = new File(savePath + "/");
     * 
     * if(!folder.exists()) folder.mkdir();
     * 
     * File file = new File(savePath, fileNameDate + "_otp.log"); BufferedReader br = null;
     * BufferedWriter bw = null; boolean isExists = file.exists();
     * 
     * try { if(isExists) { String line = ""; String oldStr = ""; String newStr = ""; br = new
     * BufferedReader(new FileReader(file)); while((line = br.readLine()) != null) { oldStr += (line
     * + "\r\n"); } newStr = oldStr + logMsg; bw = new BufferedWriter(new FileWriter(file));
     * bw.write(newStr); } else { bw = new BufferedWriter(new FileWriter(file)); bw.write(logMsg); }
     * bw.flush(); } catch (IOException e) { e.printStackTrace(); } finally { if(br != null) try
     * {br.close(); } catch (IOException e) {} if(bw != null) try {bw.close(); } catch (IOException
     * e) {} rtn_msg = "success"; }
     * 
     * return rtn_msg;
     */
  ...
  public String KTTDDNS_OTP_LOG2(Map<String, Object> request, Map<String, Object> response) 
  {
    ...
    // String msg = "OTP SEND - DOMAIN : " + domain + ", MAC ADDRESS : " + macAddress + ", SERVICE
    // NO : " + serviceNo + ", PHONE : " + phone;
    ...
    if (request.containsKey("CHANGE_OTP_YN_FORCE")) {
      ...
      if (!ddnslogService.insertDdnslog(macAddress, msg.toString())) /// OTP_YN의 경우 해당 INSERT 구문이 성공
    ...
    */
	  ...
	  //2023-10-20 OTP LOG2 access_rule 추가(김동혁선임 nvr에 전달)
	    ...
	    try {
		    ...
		    else if (!ddnslogService.insertDdnslog(macAddress, msg.toString())) 
		    ...
		    if (map != null && map.size() > 0 && 
	    		map.get(0).get("service_user") != null && 
	    		map.get(0).get("service_user").toString().equals("1") ) 
		    {
  ...
  public String KTTDDNS_MAC_TO_SERVICE(Map<String, Object> request, Map<String, Object> response) {
     ...
     * 
     * String sysid = ""; if(device.get("sys_id") != null) sysid = (String)device.get("sys_id");
     * 
     * response.put("sysid", sysid);
     */
    ///////////////////////////////////////////////////////////////////////////////////
  ...
  public String KTTDDNS_MAC_SERVICEOPEN(Map<String, Object> request, Map<String, Object> response) {
    ...
    if (device.get("service_no").toString().length() != 8 || device.get("sys_id") == null
        ...
        || (!device.get("cust_sts").toString().equals("01")
            && !device.get("cust_sts").toString().equals("04")
  ...
  public String KTTDDNS_SERVICE_APP_REGISTER(Map<String, Object> request,
      Map<String, Object> response) {
    @SuppressWarnings("unchecked")
    ...
    do {
      ...
      for (Map<String, Object> service : serviceList) {
        ...
        String contractNo =
            apiService.selectServiceContractnoWhereServiceno(service.get("serviceNo").toString());
        if (contractNo == null) {
          msg = "서비스 번호가 존재하지 않습니다.";
          break;
        }

        String serviceNoCheck = device.get("service_no").toString().replaceAll("\\s+", ""); //\\s+ -하나 이상의 공백 문자 시퀀스
      ...
      for (count = 0; count < serviceList.size(); count++) {
        if (!apiService.updateDeviceServicenoWhereMac(list[count][0], list[count][1],
          ...
          break;
        /*
         * //DDNS Bind Server's Zone (Named) File 에 신규 등록 하는 기능 if(registerType >= 3 && registerType
         * <= 4) { if(device != null && device.containsKey("fullDomain")&&
         * device.containsKey("addr")) { String domain, addr;//DDNS ZONE 파일 등록 기능 , 신규 등록이 아닌
         * IP-RENEWAL의 경우 DDNS Process에서 처리하도록 수정
         * 
         * domain=(String)device.get("fullDomain"); addr =(String)device.get("addr"); //
         * System.out.println("domain>>"+ domain); // System.out.println("addr>>"+ addr); final
         * String cmd = "/usr/local/kttzfm/user_data_ns.sh";//shell script ( DDNS 관련 스크립트 호출) if(new
         * File(cmd).exists()) { CommandLine cmdLine = new CommandLine(cmd); //(등록할 맥주소 또는 도메인 명칭)
         * cmdLine.addArgument(domain,false); cmdLine.addArgument(addr,false);
         * this.execAsync(cmdLine); } } }
         */
      }
  ...
  public String execAndRtnResult(CommandLine command) throws Exception {
    ...
    /*
     * 로컬 테스트시 사용 커멘드 CommandLine tmp = CommandLine.parse("ipconfig"); tmp.addArgument("/all");
     */
  ...
  public String KTTDDNS_SERVICE_REGISTER(Map<String, Object> request,
      Map<String, Object> response) {
    String macAddress = (String) request.get("macAddress");
    String serviceNo = (String) request.get("serviceNo");
    String empNo = (String) request.get("empNo");
    ...
    do {
      ...
      if (cust_sts == null || (!cust_sts.equals("01") && !cust_sts.equals("02")
          && !cust_sts.equals("04") && !cust_sts.equals("05"))) {
      ...
      /**
       * CODE HISTORY : 2022-10-07 registerType 이 3인 경우에만 펌웨어 업그레이드 유무를 판단하도록 변경 (종전 3과 4인 경우)
       */
      ...
      if (registerType == 3 || registerType == 4) {
        try {
          ...
          if (apiService.verString2Int(strLast_ver) == 0
              || apiService.verString2Int(strDevice_ver) == 0) {
            msg = "펌웨어 버전 표시가 잘못되었습니다.";
            break;
          }
          if (registerType == 3 && (apiService.verString2Int(strDevice_ver) < apiService
              .verString2Int(strLast_ver))) {
      ...
      /*
       * if(registerType >= 3 && registerType <= 4) { if(device.get("last_ver") == null) { msg =
       * "모델을 확인할 수 없습니다."; break; }
       * 
       ...
       * apiService.verString2Int(device.get("last_ver").toString())) { msg = "펌웨어 업그레이드를 진행해 주세요.";
       * result = "fwup"; break; } }
       */
      ...
      /*
       * //DDNS Bind Server's Zone (Named) File 에 신규 등록 하는 기능 if(registerType >= 3 && registerType
       * <= 4) { if(device != null && device.containsKey("fullDomain")&& device.containsKey("addr"))
       * { String domain, addr;//DDNS ZONE 파일 등록 기능 , 신규 등록이 아닌 IP-RENEWAL의 경우 DDNS Process에서 처리하도록
       * 수정
       * 
       * domain=(String)device.get("fullDomain"); addr =(String)device.get("addr"); //
       * System.out.println("domain>>"+ domain); // System.out.println("addr>>"+ addr); final String
       * cmd = "/usr/local/kttzfm/user_data_ns.sh";//shell script ( DDNS 관련 스크립트 호출) if(new
       * File(cmd).exists()) { CommandLine cmdLine = new CommandLine(cmd); //(등록할 맥주소 또는 도메인 명칭)
       * cmdLine.addArgument(domain,false); cmdLine.addArgument(addr,false);
       * this.execAsync(cmdLine); } } }
       */
  ...
  public String KTTDDNS_SERVICE_CHECK_OPEN(Map<String, Object> request,
      Map<String, Object> response) {
    @SuppressWarnings("unchecked")
  ...
  public String KTTDDNS_SERVICE_DEVICEINFO(Map<String, Object> request,
      Map<String, Object> response) {
    @SuppressWarnings("unchecked")
    ArrayList<String> serviceNoList = (ArrayList<String>) request.get("serviceNoList");
    ...
    do {
      ...
      List<Map<String, Object>> serviceList =
          apiService.selectServiceWhereInServiceno(serviceNoListString);
      ...
      for (Map<String, Object> tmpDevice : tmpDeviceList) {
        ...
        if (tmpDevice.get("device_ver") == null
            || tmpDevice.get("device_ver").toString().length() == 0) {
	          device.put("model", "");
	          device.put("fwUp", 1);
        } 
        else if (tmpDevice.get("last_ver") == null
        	|| tmpDevice.get("last_ver").toString().length() == 0)
        {
  ...
  public String KTTDDNS_SERVICE_UNREGISTER(Map<String, Object> request,
      Map<String, Object> response) {
    @SuppressWarnings("unchecked")
    ...
    do {
      if (deviceList == null || deviceList.size() == 0 || serviceNo == null
          || serviceNo.length() != 8)
        break;
      ...
      for (Map<String, Object> tmpDevice : tmpDeviceList) {
        ...
        if (0 <= (int) tmpDevice.get("register_type")
            && (int) tmpDevice.get("register_type") <= 2) {
  ...
  public String KTTDDNS_SERVICE_DELETE_DEVICE(Map<String, Object> request,
      Map<String, Object> response) {
    String macAddress = (String) request.get("macAddress");
    String serviceNo = (String) request.get("serviceNo");
    ...
    do {
      ...
      if (!ddnslogService.insertDdnslog(macAddress,
          String.format("KTTDDNS_SERVICE_DELETE_DEVICE - MAC ADDRESS : %s  SERVICE NO : %s",
              macAddress, serviceNo)))
        break;
  ...
  public String KTTDDNS_SERVICE_DELETE_MAC(Map<String, Object> request,
      Map<String, Object> response) {
    String macAddress = (String) request.get("macAddress");
  ...
  public String KTTDDNS_SERVICE_SERVICEOPEN(Map<String, Object> request,
      Map<String, Object> response) {
    @SuppressWarnings("unchecked")
    ArrayList<String> deviceList = (ArrayList<String>) request.get("deviceList");
...
/*
   public String KTTDDNS_SERVICENO_TO_DEVICE_PLAY_URL_CMS(Map<String, Object> request,
      Map<String, Object> response) {
    String serviceNo = (String) request.get("serviceNo");
    String eventTime = (String) request.get("eventTime");
    Integer guardStatus = (Integer) request.get("guardStatus");
    String empNo = (String) request.get("empNo");
    boolean fwVer300UnderFlag = false;
    if (serviceNo == null || serviceNo.length() != 8 || eventTime == null
        || eventTime.length() != 14 || !StringUtils.isNumeric(eventTime) || empNo == null
        || empNo.length() == 0) {
      response.put("msg", "포맷 에러 등으로 실패하였습니다.");
    ...
    List<Map<String, Object>> deviceList = new ArrayList<>();
    for (Map<String, Object> deviceItem : deviceListOrg) {
     final String apiMac = Encryptions.remakeMac((String) deviceItem.get("macAddress"), false);
     final String domain = (String) deviceItem.get("fullDomain");
     final String port = (String) deviceItem.get("port");
     ...
     final String timeStr = String.format("%08X", (System.currentTimeMillis() / 1000) + limitTime); // (60
      // *
      // (int)
      // setup.get("devplaykey_limit_min")));
      // int limitTime = 60 * (int) setup.get("devplaykey_limit_min");
      String otp1 = "";
      String chkMa = "";
      
      int device_ver = 0;
      try {
        //System.out.println(deviceItem.get("RawDeviceVer"));
        if( deviceItem.containsKey("RawDeviceVer")) {
          java.util.StringTokenizer stk = new java.util.StringTokenizer((String) deviceItem.get("RawDeviceVer"),".");
          if(stk.countTokens() == 3) {
            ...
            if(m.find())
            {
              if(Integer.valueOf(m.group()) >=3) {
                fwVer300UnderFlag = false;
              }else {
                fwVer300UnderFlag = true;
              }
          ...
          device_ver = Integer.parseInt((String) deviceItem.get("deviceVer"));
          if (device_ver < 300) fwVer300UnderFlag = true;
      ...
      if (device_ver < 3000000)
      {
        // masterkey 발급 && chkMa = 1
        otp1 = Encryptions.encryptMasterkey(Encryptions.getSHA(apiMac + "KTT_MASTER", 1));
        chkMa = "1";
      } else if(device_ver >= 3000000) {//if (device_ver >= 3.0.0)
        if (apiMac.length() != 12) {
          response.put("msg", "잘못된 맥주소입니다.");
          return "fail";
        }
        ...
        otp1 = selfasService.selectSelfasWhereMacAndUserlevelCmsCertnum(apiMac, 2); // ascode - DB에서
        // CertNum을 읽어
        // 오는 부분으로 , 위
        // 주석으로 변경 가능함.
        chkMa = "0";
      }
      String otp = Encryptions.makeOtp(otp1, timeStr);
      String url = "kttcmslite64://KTT_GUARD?domain=" + domain + "&port=" + port + "&otp=" + otp + "&eventtime="  + eventTime + "&limitTime=" + limitTime + "&guardstatus=" + guardStatus + "&chkMa=" +  chkMa;
      ...
      try (Formatter fm = new Formatter(url)) {
        ...
        //response.put("msg", "AS Code 생성에 실패하였습니다."+( logger.isDebugEnabled() ? e.getLocalizedMessage() :""));
       // return "fail";
  ...
  */
  public String KTTDDNS_SERVICENO_TO_DEVICE_PLAY_URL_CMS(Map<String, Object> request,
    ...
    deviceListOrg.parallelStream().forEach(deviceItem -> {
      Map<String, Object> device = new HashMap<>();
      final String apiMac = deviceItem.containsKey("macAddress")?Encryptions.remakeMac((String) deviceItem.get("macAddress"), false):"";
      ...
      if (apiMac.length() == 12) {
        try {
          if( deviceItem.containsKey("RawDeviceVer")) {
            final  java.util.StringTokenizer stk = new java.util.StringTokenizer((String) deviceItem.get("RawDeviceVer"),".");
            Pattern p = Pattern.compile("\\d+");
            Matcher m = p.matcher(stk.nextToken()); //가장 앞에 펌웨어 버전 자리의 수가 3이면  3.0.0이상의기준을 만족
            if(m.find())
            {
  ...
  public String KTTDDNS_CLIENT_ACCESSLOG(Map<String, Object> request,
      Map<String, Object> response) {
    ...
    try {
      ...
      if (request.containsKey("onlyReport")) //KTTDDNS_CLIENT_ACCESSLOG(통계 기록 모드)
        if (request.containsKey("mac_list")) {
          ...
          List<String> mlist =
              (List<String>) apiService.updateAppStaistics(list, app_name, mobile_type);
      ...
      try {
        ...
        if (request.containsKey("client_type") && request.containsKey("register_type")) {
          ...
          if (isCLientAPI && request.containsKey("id")) { // CLINET 용 (주로 : 모바일단말기)
            ...
            switch (intType) {
              case 0: // 통합 앱 app_date 컬럼과 app_access_id컬럼값을 갱신한다.
                  result = clientAccessLogService.insert_update_ClientAccessLogTbl(ClientAccessLogService.TYPE_APP_ACCESS_LOG, (String) request.get("id"), Encryptions.remakeMac(mac, true), String.valueOf(request.get("register_type")))> 0 ? "success"
                        : "fail";
                break;
              case 1: // cms_date 컬럼과 cms_access_id컬럼값을 갱신한다.,,RESERVED
                result = clientAccessLogService.insert_update_ClientAccessLogTbl(ClientAccessLogService.TYPE_CMS_ACCESS_LOG, (String) request.get("id"), Encryptions.remakeMac(mac, true), String.valueOf(request.get("register_type")))> 0 ? "success"
                    : "fail";
                break;
              case 2:
                result = clientAccessLogService.insert_update_ClientAccessLogTbl(ClientAccessLogService.TYPE_DEVICE_ACCESS_LOG,String.valueOf(o), Encryptions.remakeMac(mac, true), "fromDevice")> 0 ? "success"
                    : "fail";
                break;
              default:
                response.clear();
          ...
          if (isCLientAPI && request.containsKey("protocol_type")) { // CLINET 용 (주로 : 모바일단말기)
            switch (intType) {
              /*
               case 0: // 장비 UI에서 호출 된 경우
                result = clientAccessLogService.insert_update_ClientAccessLogTbl(ClientAccessLogService.TYPE_APP_ACCESS_LOG, "🖲 VideoSurveillanceDevice", Encryptions.remakeMac(mac, true), request.get("protocol_type").toString())> 0 ? "success" : "fail";
                break;
              case 1: //장비 웹 뷰어에서 호출 된 경우.
                result = clientAccessLogService.insert_update_ClientAccessLogTbl(ClientAccessLogService.TYPE_CMS_ACCESS_LOG, "🌐 WEB-VIEWER", Encryptions.remakeMac(mac, true), request.get("protocol_type").toString())> 0 ? "success"              : "fail";
      */
              case 0:
              case 1:
              result = clientAccessLogService.insert_update_ClientAccessLogTbl(ClientAccessLogService.TYPE_DEVICE_ACCESS_LOG, strType, Encryptions.remakeMac(mac, true), request.get("protocol_type").toString())> 0 ? "success"
                  : "fail";
                break;
              default:
                result = "fail";
           ...
           // String strType = (String) request.get("type");
            result = clientAccessLogService.insert_update_ClientAccessLogTbl(ClientAccessLogService.TYPE_UNKOWN_ACCESS_LOG, strType, Encryptions.remakeMac(mac, true), "fromDevice")> 0 ? "success"
                : "fail";
   ...
   * @작성자 : Foryoucom
   * @변경이력 :
   * @Method 설명 :녹화기 등 장비에서 보내는 로그로 장비가 외부 공격 등에 의하여 실행파일의 변조 등을 리포트 시 이를 특정 파일에 기록 등을 실행함.
   * @param request DDNS API 에 따른 파라미터
   * @param response success 등 전송해야 할 파라미터
   * @return
   */
  ...
  public String KTTDDNS_INFECTION_LOG(Map<String, Object> request, Map<String, Object> response) {
    ...
    if (request.containsKey("macAddress") && request.containsKey("infection_type")) {
      ...
      try (ImergencyReport imergencyReport = new ImergencyReport();) {
        ...
        infection_log_msg.append("INFECTION MSG ➡️ ").append("MAC : ").append(apiMac)
            .append(" Type : ").append(infection_type).append(" DOMAIN: ").append(domain);
        if (request.containsKey("is_recovery_infection")) {
          KTTDDNS_INFECTION_LOG_looger.info("infection : is_recovery_infection {} infection : Log {} ", request.get("is_recovery_infection"),log_data);
        ...
        if (log_data.length() > 0 && bWriteMonitoringLog) {
          ...
          for (int i = 0; i < 3; i++) {
            if (ER_Report_lock.tryLock(20, TimeUnit.SECONDS)) {
              ...
              try {
                FileOutputStream _fStream = new FileOutputStream(fd_loc, true); // location , append flag
      ...
      // RequestAttributes requestAttributes = RequestContextHolder.getRequestAttributes();
      HttpServletResponse httpServelResponse =
          ((ServletRequestAttributes) requestAttributes).getResponse();
      try {
        final CSVPrinter csvPrinter =
            new CSVPrinter(httpServelResponse.getWriter(), CSVFormat.EXCEL);
        ...
        httpServelResponse.setHeader("Content-disposition",
  ...
  private static final String ipv6Pattern2 =
  ...
  public String KTTDDNS_CHECK_IP_VALIDATION(Map<String, Object> request,
      Map<String, Object> response) {
    String result = "fail";
