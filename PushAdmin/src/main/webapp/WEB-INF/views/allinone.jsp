...
$(function()
{
	...
	$("#search_server_id_btn").click(function()
	{
		$("#search_server_id_form").submit();
	});
...
function allinone_search_create_date()
{
...
function delete_allinone(server_id, user_id)
{
	...
	do
	{
		...
		if(confirm('서버 아이디 : ' + server_id + '\n사용자 아이디 : ' + user_id + '\n삭제하시겠습니까?'))
		{
			var JSON_DATA = {"server_id" : server_id, "user_id" : user_id};
			$.ajax({
				type : "post",
				url : "delete_allinone.do",
			...
			<td>
				<center>
					<table border="0" width="100%">
						<tr>
							<td style="background-color: #F6F6F6; border: 0px; top: 5px;" height="30px" colspan="10" align="center" id="font2">A L L&nbsp;&nbsp;&nbsp;I N&nbsp;&nbsp;&nbsp;O N E</td>
						</tr>
						<tr>
							<td height="30"></td>
						</tr>
						...
						<tr>
							<td width="6%" align="center" id="font1">No.</td>
							<td width="11%" align="center" id="font1" style="cursor:pointer;" onclick="location.href='/PushAdmin/allinone_page.do?type=${type }&sort=server_id&direction=${direction.equals('desc') ? 'asc' : 'desc' }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }'">서버 아이디</td>
							<td width="10%" align="center" id="font1" style="cursor:pointer;" onclick="location.href='/PushAdmin/allinone_page.do?type=${type }&sort=user_id&direction=${direction.equals('desc') ? 'asc' : 'desc' }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }'">사용자 아이디</td>
							<td width="29%" align="center" id="font1" colspan="2">토큰</td>
							<td width="10%" align="center" id="font1" style="cursor:pointer;" onclick="location.href='/PushAdmin/allinone_page.do?type=${type }&sort=app_ver&direction=${direction.equals('desc') ? 'asc' : 'desc' }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }'">앱 버전</td>
							<td width="8%" align="center" id="font1" style="cursor:pointer;" onclick="location.href='/PushAdmin/allinone_page.do?type=${type }&sort=mobile_type&direction=${direction.equals('desc') ? 'asc' : 'desc' }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }'">모바일 종류</td>
							<td width="8%" align="center" id="font1" style="cursor:pointer;" onclick="location.href='/PushAdmin/allinone_page.do?type=${type }&sort=app_num&direction=${direction.equals('desc') ? 'asc' : 'desc' }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }'">앱 종류</td>
							<td width="14%" align="center" id="font1" style="cursor:pointer;" onclick="location.href='/PushAdmin/allinone_page.do?type=${type }&sort=create_date&direction=${direction.equals('desc') ? 'asc' : 'desc' }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }'">등록일</td>
						...
						<c:forEach var="allinone_list" items="${allinone_list }">
							<tr height="40">
								<td align="center">${allinone_list.row_idx }</td>
								<td align="center">${allinone_list.server_id }</td>
								<td align="center">${allinone_list.user_id }</td>
								<td align="center" colspan="2" style="word-break:break-all"><strong style="font-size: 11px;">${allinone_list.token_id }</strong></td>
								<td align="center">${allinone_list.app_ver }</td>
								<td align="center">
									<c:choose>
										<c:when test="${allinone_list.mobile_type == 1}">Android</c:when>
										<c:when test="${allinone_list.mobile_type == 2}">iOS</c:when>
										<c:when test="${allinone_list.mobile_type == 3}">iOS-FCM</c:when>
										<c:otherwise>${allinone_list.mobile_type }</c:otherwise>
									</c:choose>
								</td>
								<td align="center">
									<c:choose>
										<c:when test="${allinone_list.app_num == 11}">All In One</c:when>
										<c:when test="${allinone_list.app_num == 10}">OCT INAPP</c:when>
										<c:when test="${allinone_list.app_num == 9}">OCT Plus</c:when>
										<c:when test="${allinone_list.app_num == 7}">Sea Black Box</c:when>
										<c:when test="${allinone_list.app_num == 6}">Olleh CCTV Telecop</c:when>
										<c:when test="${allinone_list.app_num == 0}"></c:when>
										<c:otherwise>${allinone_list.app_num }</c:otherwise>
									</c:choose>
								</td>
								<td align="center">${allinone_list.create_date }</td>
								<td align="center">
									<c:if test="${login_member.member_auth == 2 }">
										<input type="button" id="delete_btn" style="background: #000000; color: white;" value="삭제" onclick="javascript:delete_allinone('${allinone_list.server_id }', '${allinone_list.user_id }')">
									</c:if>
								</td>
							</tr>
							<tr height="0.1">
						...
						<tr>
							<td colspan="10" align="center">
								<c:if test="${start_page > 10 }">
									<a href="/PushAdmin/allinone_page.do?type=${type }&sort=${sort }&direction=${direction }&page=${1 }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }">[맨앞]</a>
									<a href="/PushAdmin/allinone_page.do?type=${type }&sort=${sort }&direction=${direction }&page=${start_page - 10 }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }">[이전]</a>
								</c:if>
								<c:forEach var="a" begin="${start_page }" end="${end_page <= last_page ? end_page : last_page }">
									<c:choose>
										...
										<c:otherwise>
											<a href="/PushAdmin/allinone_page.do?type=${type }&sort=${sort }&direction=${direction }&page=${a }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }">[${a }]</a>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								<c:if test="${end_page < last_page }">
									<a href="/PushAdmin/allinone_page.do?type=${type }&sort=${sort }&direction=${direction }&page=${start_page + 10 }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }">[다음]</a>
									<a href="/PushAdmin/allinone_page.do?type=${type }&sort=${sort }&direction=${direction }&page=${last_page }&server_id=${server_id }&user_id=${user_id }&mobile_type=${mobile_type }&start_create_date=${start_create_date }&end_create_date=${end_create_date }">[맨뒤]</a>
								</c:if>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="3">
								<form id="search_server_id_form" name="search_server_id_form" action="/PushAdmin/allinone_page.do">서버 아이디&nbsp;&nbsp;
									<input id="server_id" name="server_id" style="vertical-align: 5%" value="${server_id }">
									<input id="search_server_id_btn" type="button" style="background: #000000; color: white;" value="검색">
									<input type="hidden" name="type" id="type" value="allinone_search_server_id">
								</form>
							</td>
							<td colspan="3" align="right"> 
								<form id="excel_form" name="excel_form" action="/PushAdmin/allinone_excel.xlsx">엑셀 다운로드&nbsp;&nbsp;
								...
								<form id="excel_all_form" name="excel_all_form" action="/PushAdmin/allinone_excel.xlsx">
									<c:choose>
										<c:when test="${type == 'allinone' }">
											<input type="hidden" name="type" id="type" value="allinone_all">
										</c:when>
										<c:when test="${type == 'allinone_search_user_id' }">
											<input type="hidden" name="type" id="type" value="allinone_search_user_id_all">
											<input type="hidden" name="user_id_excel" id="user_id_excel" value="${user_id }">
										</c:when>
										<c:when test="${type == 'allinone_search_mobile_type' }">
											<input type="hidden" name="type" id="type" value="allinone_search_mobile_type_all">
											<input type="hidden" name="mobile_type_excel" id="mobile_type_excel" value="${mobile_type }">
										</c:when>
										<c:when test="${type == 'allinone_search_create_date' }">
											<input type="hidden" name="type" id="type" value="allinone_search_create_date_all">
											<input type="hidden" name="start_create_date_excel" id="start_create_date_excel" value="${start_create_date }">
											<input type="hidden" name="end_create_date_excel" id="end_create_date_excel" value="${end_create_date }">
										</c:when>
									</c:choose>
						...
						<tr>
							<td colspan="4" height="30"></td>
							<td colspan="3">
								<form id="search_user_id_form" name="search_user_id_form" action="/PushAdmin/allinone_page.do">사용자 아이디&nbsp;&nbsp;
									...
									<input type="hidden" name="type" id="type" value="allinone_search_user_id">
								</form>
							</td>
							<td colspan="3" align="right">
								<input type="button" onclick="location.href='/PushAdmin/allinone_page.do'" style="background: #000000; color: white;" value="전체 목록">
							</td>
						</tr>
						<tr>
							<td colspan="4" height="30"></td>
							<td colspan="6">
								<form id="search_mobile_type_form" name="search_mobile_type_form" action="/PushAdmin/allinone_page.do">모바일 종류&nbsp;&nbsp;
									...
									<input type="hidden" name="type" id="type" value="allinone_search_mobile_type">
								</form>
							</td>
						</tr>
						<tr>
							...
							<td colspan="6">등록일&nbsp;&nbsp;
								...
								<input type="button" style="background: #000000; color: white;" value="검색" onclick="javascript:allinone_search_create_date()">
								<form action="/PushAdmin/allinone_page.do" id="search_create_date_form">
									...
									<input type="hidden" name="type" id="type" value="allinone_search_create_date">
								</form>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td style="background-color: #F6F6F6; border: 0px; top: 5px;" height="30px" colspan="10" align="center" id="font2">P U S H&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;S E R V E R&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;A D M I N I S T R A T O R</td>
						</tr>
					</table>
				</center>
			</td>
